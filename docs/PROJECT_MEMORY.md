...
- Startup Performance Optimization P1(시작 성능 최적화 1차): 구현 완료 / Build 97 실기기 QA 대기. `/api/moments` stale cleanup을 blocking path에서 분리했고, raw token 저장 없이 bearer token SHA-256 hash 기반 짧은 TTL cache로 `resolveRequestUser` 반복 비용을 줄였으며, thumbnail signed URL wall time 계측을 추가했다
...
- Build 97 Startup Performance Optimization P1 QA(빌드 97 시작 성능 최적화 1차 QA): EAS preview/internal build 완료 / Founder timing QA 대기. Build commit `1bb347c`, iOS buildNumber `97`, EAS Build ID `a3693975-e234-4ae0-a169-373fd683cd3a`. Install page는 `https://expo.dev/accounts/jspark88/projects/action-sports-journal/builds/a3693975-e234-4ae0-a169-373fd683cd3a`, IPA URL은 `https://expo.dev/artifacts/eas/46cVuinLZ-VVowkdVFcw-iKcdjx-vvzG10RU4M7Vyx4.ipa`. 다음 시작점은 Startup Performance Optimization P1 QA 결과 확인이다. Build 96 대비 app `apiMs`, app `serverTotalMs`, Render `cacheHit`, `staleCleanupBlocking=false`, `authGetUserMs`, `publicUserLookupMs`, `publicUserUpsertOrSyncMs`, `staleCleanupMs`, `momentsQueryMs`, `evidenceQueryMs`, `thumbnailSignedUrlMs`, `thumbnailSignedUrlWallMs`, `responseBytes`를 비교한다
- Startup Performance Optimization P1 QA(시작 성능 최적화 1차 QA): 구현 완료 / Build 97 QA 대기. Build 96에서 확인한 server-side 지연을 줄이기 위해 stale cleanup 비동기화, request user TTL cache, thumbnail wall timing 보정을 반영했다. Build 97에서 0개 계정 `serverTotalMs`가 얼마나 줄었는지 확인한다
...
QA / 검증 대기:
- Build 97 Startup Performance Optimization P1 QA(빌드 97 시작 성능 최적화 1차 QA): EAS preview/internal build 완료 / timing QA 대기. Build commit `1bb347c`, iOS buildNumber `97`, EAS Build ID `a3693975-e234-4ae0-a169-373fd683cd3a`. Install page는 `https://expo.dev/accounts/jspark88/projects/action-sports-journal/builds/a3693975-e234-4ae0-a169-373fd683cd3a`, IPA URL은 `https://expo.dev/artifacts/eas/46cVuinLZ-VVowkdVFcw-iKcdjx-vvzG10RU4M7Vyx4.ipa`. 다음 시작점은 Startup Performance Optimization P1 QA 결과 확인이다. 확인 값은 app `apiMs`, app `serverTotalMs`, Render `cacheHit`, `resolveRequestUserMs`, `authGetUserMs`, `publicUserLookupMs`, `publicUserUpsertOrSyncMs`, `staleCleanupBlocking=false`, `staleCleanupMs`, `momentsQueryMs`, `evidenceQueryMs`, `thumbnailSignedUrlWallMs`, `thumbnailSignedUrlMs`, `responseBytes`다.
