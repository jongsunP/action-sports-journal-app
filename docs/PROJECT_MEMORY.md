...
- Startup Performance Optimization P1(시작 성능 최적화 1차): 구현 완료 / 다음 build QA 대기. `/api/moments` stale cleanup을 blocking path에서 분리했고, raw token 저장 없이 bearer token SHA-256 hash 기반 짧은 TTL cache로 `resolveRequestUser` 반복 비용을 줄였으며, thumbnail signed URL wall time 계측을 추가했다
...
- Startup Performance Optimization P1 QA(시작 성능 최적화 1차 QA): 구현 완료 / 다음 build 필요. Build 96에서 확인한 server-side 지연을 줄이기 위해 stale cleanup 비동기화, request user TTL cache, thumbnail wall timing 보정을 반영했다. 다음 build에서 0개 계정 `serverTotalMs`가 얼마나 줄었는지 확인한다
...
QA / 검증 대기:
- Startup Performance Optimization P1 QA(시작 성능 최적화 1차 QA): 구현 완료 / 다음 preview/internal build QA 대기. 확인 값은 app `apiMs`, app `serverTotalMs`, Render `cacheHit`, `resolveRequestUserMs`, `staleCleanupBlocking=false`, `staleCleanupMs`, `thumbnailSignedUrlWallMs`, `thumbnailSignedUrlMs`, `responseBytes`다.
