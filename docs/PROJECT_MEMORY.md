...
- Startup Performance Optimization P1.6 QA(시작 성능 최적화 1.6차 QA): 구현 완료 / 다음 build QA 필요. 확인 값은 7-record 반복 접속에서 `thumbnailSignedUrlCacheHits`, `thumbnailSignedUrlCacheMisses`, `thumbnailSignedUrlWallMs`, `thumbnailSignedUrlMs`, `serverTotalMs`가 Build 98 대비 줄어드는지다.
