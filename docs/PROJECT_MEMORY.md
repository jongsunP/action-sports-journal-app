...
- Startup Performance Optimization P1.5 QA(시작 성능 최적화 1.5차 QA): 구현 완료 / 다음 build QA 필요. list에서 compact evidence만 반환하고 Detail에서 full evidence를 별도 조회하는 최소 list/detail payload 분리를 적용했다. 다음 build에서 7개 계정 `responseBytes`와 `serverTotalMs`가 Build 97 대비 얼마나 줄었는지 확인한다
...
- Startup Performance Optimization P1.5 QA(시작 성능 최적화 1.5차 QA): 구현 완료 / 다음 standalone QA 필요. 확인 값은 7개 계정 `responseBytes`, `serverTotalMs`, `evidenceQueryMs`, `normalizationMs`, Detail 진입 full evidence 보강 여부다.
