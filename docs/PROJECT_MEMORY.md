...
- Startup Performance Optimization P1.5 QA(시작 성능 최적화 1.5차 QA): 구현 완료 / 다음 build QA 필요. list에서 compact evidence만 반환하고 Detail에서 full evidence를 별도 조회하는 최소 list/detail payload 분리를 적용했다. Moment Detail QA/debug에서 detail request id/server ms/fetch ms/response bytes를 볼 수 있다. 다음 build에서 7개 계정 list `responseBytes`/`serverTotalMs`와 Detail fetch diagnostics를 Build 97 대비 확인한다
...
QA / 검증 대기:
- Development Build / Local Build Workflow(개발 빌드 / 로컬 빌드 워크플로우): 다음 시작점. 반복 EAS preview/internal build 비용을 줄이고, native dependency가 있는 기능도 더 빠르게 검증할 수 있는 workflow를 검토한다.
