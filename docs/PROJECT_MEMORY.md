...
- Startup Performance P2 Summary-first Boot(시작 성능 2차 summary-first 부팅): 구현 완료 / 다음 standalone build QA 필요. `/api/moments?view=summary`를 추가했고 기본 `/api/moments`는 full로 유지해 Build 98 호환을 보존했다. Summary view는 response shape를 유지하되 list evidence lookup과 thumbnail signed URL 생성을 생략한다. Boot sync, Video first page/pagination, remote refresh, upload reconciliation lookup은 summary view를 사용한다. Detail은 기존 `GET /api/moments/:momentId` full endpoint로 evidence/thumbnail을 보강한다. Auth/ownership/no-token/DB/Storage/Upload/Recovery/AI flow는 변경하지 않았다
...
- Startup Performance Optimization P1.6 QA(시작 성능 최적화 1.6차 QA): Render 배포 완료 / Build 98 앱으로 실사용 관찰 대기. 확인 값은 7-record 반복 접속에서 `thumbnailSignedUrlCacheHits`, `thumbnailSignedUrlCacheMisses`, `thumbnailSignedUrlWallMs`, `thumbnailSignedUrlMs`, `serverTotalMs`가 Build 98 대비 줄어드는지다. 0-record 첫 접속 지연은 thumbnail cache로 해결되는 영역이 아니라 `resolveRequestUser` cache-miss 경로로 분리해서 본다.
- Startup Performance Optimization P1.7 QA(시작 성능 최적화 1.7차 QA): 구현 완료 / Render 배포 후 Build 98 앱으로 실사용 관찰 대기. 확인 값은 반복 실행/재진입에서 `cacheHit=true` 비율 증가, `resolveRequestUserMs` 감소, `authGetUserMs`/`publicUserLookupMs`/`publicUserUpsertOrSyncMs` 0ms cache path 증가, `serverTotalMs` 완화 여부다.
- Startup Performance Optimization P1.8 QA(시작 성능 최적화 1.8차 QA): 구현 완료 / Render 배포 후 Build 98 앱으로 실사용 관찰 대기. `/health.performanceCaches.requestUserCacheTtlMs=1800000`, `/health.performanceCaches.thumbnailSignedUrlCacheTtlMs=1800000` 확인 후, 30분 내 재진입에서 `cacheHit=true`, `resolveRequestUserMs=0-1ms`, `thumbnailSignedUrlWallMs=0-1ms`, `serverTotalMs` 완화 여부를 본다.
- Startup Performance Optimization P1.9 QA(시작 성능 최적화 1.9차 QA): 구현 완료 / Render 배포 후 Build 98 앱으로 실사용 관찰 대기. 확인 값은 token-hash cache miss에서도 `authUserPublicUserCacheHit=true`가 늘고 `publicUserLookupMs=0`, `publicUserUpsertOrSyncMs=0`, `publicUserSyncAction=none|deferred`가 보이는지, 그리고 evidence/thumbnail 병렬화 후 7-record cold `serverTotalMs`가 줄어드는지다.
- Startup Performance P2 Summary-first Boot QA(시작 성능 2차 summary-first 부팅 QA): 구현 완료 / 다음 standalone build 필요. 확인 값은 boot/Video 요청 로그의 `view=summary`, `evidenceQueryMs=0`, `thumbnailSignedUrlWallMs=0`, Home/Video first paint 속도, thumbnail placeholder 품질, Detail 진입 시 full evidence/thumbnail 보강 여부다.
...
- Startup Performance Optimization P1.5 QA(시작 성능 최적화 1.5차 QA): 구현 완료 / 다음 standalone QA 필요. 확인 값은 7개 계정 list `responseBytes`, `serverTotalMs`, `evidenceQueryMs`, `normalizationMs`, Detail 진입 full evidence 보강 여부, `detailFetchMs`, `detailServerTotalMs`, `detailRequestId`, `detailResponseBytes`다.
- Build 96 Startup Performance Observability P2.1 QA(빌드 96 시작 성능 관측 2.1차 QA): EAS preview/internal build 완료 / timing QA 대기. Build commit `4f8f4a2`, iOS buildNumber `96`, EAS Build ID `68b17987-b5f8-4a6f-9a06-7a2260c69708`. Install page는 `https://expo.dev/accounts/jspark88/projects/action-sports-journal/builds/68b17987-b5f8-4a6f-9a06-7a2260c69708`, IPA URL은 `https://expo.dev/artifacts/eas/JgPGyJ5Y3njSGS6-Q6MFc7zdpwjNFqmA6StePZzUssQ.ipa`. 판단 기준은 app `apiMs` ≈ app `serverTotalMs`이면 server/Supabase/Auth/Storage 경로, app `apiMs`만 크면 network/response/client 처리 쪽이다.
- Build 95 Startup Performance Observability P2 QA(빌드 95 시작 성능 관측 2차 QA): EAS preview/internal build 완료 / timing QA 대기. Build commit `f49481e`, iOS buildNumber `95`, EAS Build ID `b45e226d-60f7-458d-ab2e-e814f33ca6c6`. Install page는 `https://expo.dev/accounts/jspark88/projects/action-sports-journal/builds/b45e226d-60f7-458d-ab2e-e814f33ca6c6`, IPA URL은 `https://expo.dev/artifacts/eas/DP8-ZCgYSgzIFm1KnPRst7kmiJ3Ld_TCvzlHyTekFoY.ipa`. 판단 기준은 app `apiMs` ≈ server `serverTotalMs`이면 server/Supabase/Auth/Storage 경로, app `apiMs`만 크면 network/response/client 처리 쪽이다.
