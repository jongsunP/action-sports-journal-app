...
- EAS preview `EXPO_PUBLIC_AI_ANALYSIS_ENDPOINT` points to the Singapore
  analysis endpoint.
- iOS buildNumber `102` has been built.
...
- Audit follow-up backlog separation:
  - dev-server low-risk decomposition has started with structure-only helper
    ...
    now live in `dev-server/features/upload/uploadPolicy.ts`; Expo Push
    observability helpers now live in
    `dev-server/features/push/pushObservability.ts`. Continue dev-server
    ...
    Recovery route response/DB insert shape, upload policy response contract,
    signed upload URL generation, Push token registration/delivery,
    `push_delivery_attempts` DB shape, or AI prompt/parser schemas before AI
    Calibration.
...
- Build 94 is the current Startup Performance Observability standalone QA
  ...
  multi-day real-device observation is pending. Use QA Debug Video `api/source`
  plus `ui/norm/bootReuse/dupBlocked` and server `/api/moments` timing logs to
  decide whether the 4-6 second Video ready time is API latency, thumbnail URL
