...
- Audit follow-up backlog separation:
  - HomeScreen low-risk decomposition has started with a structure-only QA Debug
    ...
    split, followed by a thumbnail hydration hook extraction. Continue only in
    small, behavior-preserving steps; do not start a large HomeScreen split
    before AI Calibration unless a specific bug requires it.
