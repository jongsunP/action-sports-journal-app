...
import {
  buildLocalOnlyUploadSessionExpiredLog,
  buildRecoverableOrphanRecoveryFailureLog,
  buildRecoverableOrphanRecoveryStartedLog,
  buildRecoverableOrphanRecoverySuccessLog,
  buildRemoteMomentReconciledLog,
  buildRemoteMomentUnmatchedLog,
  buildUploadFailureRemoteReconcileExistingLog,
  buildUploadFailureRemoteReconcileFailedLog,
  buildUploadFailureRemoteReconcileMatchedLog,
  buildUploadFailureRemoteReconcileUnmatchedLog,
  buildUploadReconciliationCandidateCreatedLog,
  buildUploadReconciliationCandidateTargetedLog,
  hasRecoverableUploadTarget,
...
export function HomeScreen() {
  ...
  const upsertUploadReconciliationCandidate = useCallback(
    (candidate: UploadReconciliationCandidate) => {
      ...
      console.info(
        ...
        buildUploadReconciliationCandidateCreatedLog(candidate),
      );
    },
    [],
  );
    ...
    (remoteMoments: RemoteMomentRecord[]) => {
      ...
      const expiredSessionIds = Object.values(
        ...
        .filter((candidate) => {
          ...
          const ttlMs = hasRecoverableUploadTarget(candidate)
  ...
  const expireLocalOnlyOptimisticSessions = useCallback(() => {
    ...
    for (const sessionId of expiredSessionIds) {
      ...
      console.info(
        '[moment_reconciliation]',
        buildLocalOnlyUploadSessionExpiredLog(sessionId),
      );
    }
        ...
        if (matchedMoment) {
          console.info(
            ...
            buildUploadFailureRemoteReconcileMatchedLog({
              attempt,
              localSessionId,
              matchedMomentId: matchedMoment.remoteMomentId,
              reason,
              stage,
              uploadId,
            }),
...
function createHomeThemeStyles(colors: AppThemeColors, mode: 'dark' | 'light') {
  ...
  return StyleSheet.create({
    ...
    detailHydrationPreviewCard: {
      backgroundColor: colors.surfaceElevated,
      borderColor: borderSoft,
    },
    detailHydrationImage: { backgroundColor: surfaceSubtle },
    detailHydrationAnalysisCard: { borderBottomColor: borderSoft },
...
const baseStyles = StyleSheet.create({
  ...
  recentThumbFallbackLineWide: {
    backgroundColor: 'rgba(203, 213, 225, 0.16)',
    borderRadius: 999,
    height: 6,
    width: 58,
  },
  recentThumbFallbackLineNarrow: {
  ...
  detailHydrationAnalysisCard: {
    borderBottomColor: 'rgba(255, 255, 255, 0.1)',
    borderBottomWidth: StyleSheet.hairlineWidth,
    marginHorizontal: 16,
    paddingVertical: 16,
  },
  detailHydrationLineFull: {
    backgroundColor: 'rgba(203, 213, 225, 0.1)',
    borderRadius: 999,
    height: 9,
    width: '86%',
  },
  detailHydrationLineMid: {
  ...
  detailMediaPlaceholderLineNarrow: {
    backgroundColor: 'rgba(203, 213, 225, 0.1)',
    borderRadius: 999,
    height: 8,
    marginTop: 9,
